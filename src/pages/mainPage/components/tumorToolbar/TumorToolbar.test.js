...
describe("Tumor Toolbar", () => {
  ...
  it("if TumorToolbar opened, check 'Save' data from inputNumber ", () => {
    
    const handelAction = jest.fn(() => 2);
    
    const { getByTestId, store } = renderWithRedux(<TumorInfoSelect onAction={handelAction}/>);
    const saveIcon = getByTestId("tumor-save-inputNumber");
    expect(saveIcon).toBeInTheDocument();
    fireEvent.click(saveIcon);
  
    console.log(store.getState().variants.tumorInfo);
